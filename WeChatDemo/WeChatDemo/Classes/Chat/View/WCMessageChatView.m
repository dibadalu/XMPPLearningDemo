...
- (void)setMessageModelFrame:(WCMessageModelFrame *)messageModelFrame{
    ...
    self.timeLabel.text = messageModelFrame.messageModel.time;
    
    //2头像的frame
    if(messageModelFrame.messageModel.isCurrentUser){  //如果是自己
        
       
        UIImage *head = messageModelFrame.messageModel.headImage?[UIImage imageWithData:messageModelFrame.messageModel.headImage]:[UIImage imageNamed:@"DefaultProfileHead_qq"];
        self.headImage.image = head;
        
    }else{  //如果是聊天的用户
        self.headImage.image = messageModelFrame.messageModel.otherPhoto?messageModelFrame.messageModel.otherPhoto:[UIImage imageNamed:@"DefaultProfileHead"];
    }
    self.headImage.frame = messageModelFrame.headF;
    
    //3.内容的frame
    [self.contentBtn setTitle:messageModelFrame.messageModel.body forState:UIControlStateNormal];
